...
export class Schema {
    ...
    createCache() {
        ...
        for (let t of this.info.tables) {
            for (let c of t.columns) {
                let key = t.dbname + ":" + t.dbo + ":" + t.name + ":" + this.getTableColAlias(c);
    ...
    getTable(dbName: string, tableSchema: string, tableName: string): ITable {
        let key = dbName + ":" + tableSchema + ":" + tableName;
        let ret = this.tableByDbAndTableName[key];
        if (!ret)
            throw new Error(`Schema.getTable(): table '${dbName}'.'${tableSchema}'.'${tableName}' not found in database '${dbName}' `);
        return ret;
    }
    ...
    getTableColumnByAlias(table: ITable, colAlias: string): IColumn {
        let key = table.dbname + ":" + table.dbo + ":" + table.name + ":" + colAlias;
    ...
    createGraphQLSchema(): string {
        ...
        defStr.push("scalar JSON");
        queryStr.push("ping:JSON");
        ...
        for (let table of this.info.tables) {
            ...
            for (let col of table.columns) {
                ...
                if (col.type == "ObjectValue") {  // fk
                    if (col.ref_table) {
                        let refTable = this.getTable(col.ref_db, col.ref_schema, col.ref_table);
        ...
        console.log("===================================================")
        console.log(defStr.join(" ") + ` type Query {${queryStr.join(" ")}}`)
        console.log("===================================================")
...
where 
  ...
  TABLE_NAME=${stringAsSql(db.type, table_name)} order by ORDINAL_POSITION`;
    ...
    upsertTable(table: ITable) {
        let index = this.info.tables.findIndex((_table) => _table.dbname == table.dbname && _table.name == table.name && _table.dbo == table.dbo);
        if (index == -1) {
            table.version = 1;
